...
Pa.P_lon = tf([1], [(Pa.mc+2*Pa.mw), 0, 0]);
Pa.C_lon = tf([(Pa.kd_h+Pa.kp_h*Pa.sigma),(Pa.kp_h+Pa.ki_h*Pa.sigma),Pa.ki_h],[Pa.sigma,1,0]);
Plant = Pa.P_lon*Pa.C_lon;
...
% Control Design
C = tf(1, 1);
C = add_control_lead(C, 3, 30);
C = add_control_lead(C, 0.9, 4);
% C = add_control_lag(C, 0.9, 10);     
% C = add_control_proportional(C, -10);
C = add_control_lpf(C, 0.1);
...
figure(4), clf
    subplot(3,1,1), 
        ...
        bodemag(Pa.P_lon*Pa.C_lon/(1+Pa.P_lon*Pa.C_lon))
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Pa.C_longitudinal = Pa.C_lon * C;
[Pa.num_C_lon, Pa.den_C_lon] = tfdata(Pa.C_longitudinal,'v');
[Pa.A_C_lon,Pa.B_C_lon,Pa.C_C_lon,Pa.D_C_lon]=tf2ss(Pa.num_C_lon, Pa.den_C_lon);
